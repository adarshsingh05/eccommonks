...
const faqs = [
    ...
    a: (
      <>
        ...
        <ul className="list-none ml-0 mt-2 flex flex-wrap gap-2">
          <li className="bg-green-100/60 px-3 py-1 rounded-xl flex items-center gap-2">🛒 Amazon, Flipkart, Shopify, Meesho, Nykaa, JioMart</li>
          <li className="bg-green-100/60 px-3 py-1 rounded-xl flex items-center gap-2">🚀 Blinkit, Zepto, Instamart (Q-commerce)</li>
          <li className="bg-green-100/60 px-3 py-1 rounded-xl flex items-center gap-2">🛍️ Myntra, and others by request</li>
        </ul>
      </>
    ),
  },
  ...
  },
  {
    ...
    icon: <HelpCircle className="w-7 h-7 text-green-500" />,
    a: (
      <>
  ...
  },
  {
    ...
    a: (
      <>
        ...
        <span className="font-bold text-green-500">🔔 This is application-based. Details on Contact Page</span>
      </>
    ),
  },
  {
  ...
  },
  {
    ...
    a: (
      <>
        ...
        <div className="mt-2 font-bold text-green-600">Book Now →</div>
      </>
    ),
  },
];
...
function FAQCard({ q, a, icon, isOpen, onClick }) {
  return (
    <div className="w-full p-0 overflow-hidden bg-gradient-to-br from-white to-green-50/30 shadow-xl mb-4 rounded-2xl border border-green-100">
      <button
      ...
      >
        ...
        <span className="font-black text-lg sm:text-xl text-green-700 group-hover:text-green-500 transition-colors duration-300 flex-1">{q}</span>
        <span className={`ml-4 transition-transform duration-300 ${isOpen ? 'rotate-90 text-green-600' : 'text-green-400'}`}>▶</span>
      </button>
      <AnimatePresence initial={false}>
        {isOpen && (
          <motion.div
            className="px-6 pb-6 pt-0 text-green-700 text-base sm:text-lg bg-gradient-to-br from-green-50/40 to-green-200/20"
            key="content"
